...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.stock==0:
            ...
            self.left_fund=self.balance-self.price
            self.stock-=1
            self.balance=0
            if self.left_fund==0:
                return f'Here is your {self.product}.'
            return f'Here is your {self.product} and ${self.left_fund} change.'                               
...
def two_list(vals, counts):
    ...
    for i in range(len(vals)):
        new_list+=counts[i]*[vals[i]] 
    for k in range(len(new_list)):
        final_link=Link(new_list[len(new_list)-k-1],final_link)
    return final_link
class Link:
    """A linked list.
