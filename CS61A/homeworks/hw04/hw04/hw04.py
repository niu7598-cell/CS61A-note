...
def balanced(m):
    ...
    if is_planet(m):
        ...
        l1=length(arm1)
        l2=length(arm2)
        if  l1*total_mass(end(arm1)) == l2*total_mass(end(arm2)):
...
def berry_finder(t):
    ...
    if label(t)=='berry':
        return True
    elif any(berry_finder(i) for i in branches(t))  :#看看any的格式
        return True
    else: 
        return False  
