...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            assert place.ant is  None, 'Too many ants in {0}'.format(place)   #如果条件为假 (False)：程序立刻停止，抛出一个 AssertionError 异常，并打印出后面的报错信息。
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not getattr(self,"damage_doubled",False):
            self.damage*=2
