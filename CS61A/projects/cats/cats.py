...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"## 这里的错误自己都难绷 列表索引用'[]' 函数调用使用'()'
    list_correct=[i for i in paragraphs if select(i)]
    if len(list_correct)>k: 
        return list_correct[k]
    else:
        return ''
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def judgement(sentence):
        new_sentence=split(lower(remove_punctuation(sentence)))
        common= list (set(new_sentence) & set(subject) )# 这个牛逼啊！ 
        if len(common)>0:
            return True
        else:
            return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3 这两个列表的顺序要对应才行 所以这里用交集不太合适 要取出type一一对应 source
    "*** YOUR CODE HERE ***"
    k=0
    n=len(typed_words)
    m=len(source_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    'testing'
    """
    # BEGIN PROBLEM 5 先判断这个词是否正确 若正确 就直接返回 不正确再选wordlist里面的词最相似的
    # 最相似由 diff_function 定义 diff_function返回的值如果大于limit 会直接返回原来的单词
    "*** YOUR CODE HERE ***"
    if typed_word in word_list:
            return typed_word
    else:
        least_diff=min(diff_function(typed_word,x,limit) for x in word_list)
        least_list=[i for i in word_list if diff_function(typed_word,i,limit)==least_diff]
        if least_diff>limit:
            return typed_word
        else:
            return least_list[0]
...
def furry_fixes(typed, source, limit):
    ...
    credit=abs(len(typed)-len(source))
    def check(typed,source,n,limit,credit):
        ...
        if n==0:
            ...
            if typed[n-1]!=source[n-1]:
                credit+=1
                return 1 + check(typed,source,n-1,limit,credit)
            else: 
                return  check(typed,source,n-1,limit,credit)
    return check(typed,source,n,limit,credit)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """  # 加/替换/删除一个字母记作一个操作
    # 从第一个开始比较 依靠程序来抉择 原理是利用递归 
    # 想想有哪些base case
    # 终止情况 某一个列表跑完了 剩下的次数就是长度的差值
    # 超过limit
    # 完全相同 return0
    # 会有本来小于limit 但是加上长度差后大于的情形吗

    if typed==source:
        return 0
    elif limit<abs(len(typed)-len(source)):
        return limit+1
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[list_[i+1]-list_[i] for i in range(len(list_)-1)] for list_ in tpp]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    #最后要的是word 对于每一个word_index都要返回一个player

    "*** YOUR CODE HERE ***"
    lst=[[] for _ in range(len(times))]
    for word_index in word_indices:
        least_time_player=min(
            (player_num for player_num in player_indices), 
            key=lambda player_num: get_time(times,player_num,word_index)
        )
        lst[least_time_player].append(words[word_index])
    return lst
